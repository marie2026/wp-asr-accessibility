...
class ASR_REST {
	...
	public static function register_routes() {
		...
		// CORRECTION: Ajouter permission_callback pour éviter CORS ouvert
		register_rest_route( 'asr/v1', '/status/(?P<id>\d+)', array(
			'methods'             => WP_REST_Server::READABLE,
			'callback'            => array( __CLASS__, 'handle_status_request' ),
			'permission_callback' => array( __CLASS__, 'status_permission_callback' ),
		) );
	}
	...
	public static function permission_callback( $request ) {
		...
		// 3 uploads par heure max pour anonymes
		if ( $count >= 3 ) {
	...
	/**
	 * CORRECTION: Permission callback pour l'endpoint de statut (protection CORS)
	 */
	public static function status_permission_callback( $request ) {
		$nonce = $request->get_header( 'x-wp-nonce' );
		if ( $nonce && wp_verify_nonce( $nonce, 'wp_rest' ) ) {
			return true;
		}
		// Autoriser aussi les utilisateurs connectés
		return is_user_logged_in();
	}

	/**
	 * CORRECTION: Fonction unique (suppression de la duplication)
	 */
